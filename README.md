...
应用路由采用与 GG 相近的声卡分区布局：Game、Chat、Media、Aux 分别作为路由卡片，每张卡片列出当前实际连接到该声卡的应用。可直接把应用拖到另一张卡片来切换该进程的默认输出；拖动时会显示应用浮层、来源与目标声卡提示。面板显示期间每 2 秒从 Sonar 回读一次状态，因此 GG 中的修改也会同步显示。OBS 这类同时建立多条音频会话的应用会出现在所有实际连接的声卡下，不会被错误折叠成单一路由。
